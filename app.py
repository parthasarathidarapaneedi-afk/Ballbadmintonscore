...
def snapshot(s):
    # Store only score/serve state — NOT the full history list (avoids exponential memory growth)
    d = asdict(s)
    d["history"] = []   # ← KEY FIX: never nest history inside history
    return d
...
# ===================== SESSION INIT =====================
# ── Restore from file if session was reset by Streamlit Cloud ──
def try_restore_from_file():
    """Called once per session. Restores match state from disk if session was wiped."""
    try:
        if os.path.exists("match_state.json"):
            with open("match_state.json", "r") as f:
                payload = json.load(f)
            if payload.get("match"):
                m = MatchState(**payload["match"])
                st.session_state.match = m
                st.session_state.setup_done = True
            if payload.get("match_history"):
                st.session_state.match_history = payload["match_history"]
    except Exception:
        pass  # If restore fails, start fresh — never crash
...
# Restore once per session on first load
...
def save_match_to_file():
    """Persist current match + history to JSON so data survives Streamlit reruns."""
    try:
        m = st.session_state.match
        payload = {
            "match": snapshot(m),
        ...
        pass  # Never crash the app due to save failure

def add_point(winner):
    m = st.session_state.match
    if m.match_over: return

    # ── FIX 1: limit history to 20 snapshots (not 300) to prevent memory blowup
    ...
    if winner == "A":
        ...
        if srv and srv in m.player_points_A:
            m.player_points_A[srv] = m.player_points_A.get(srv, 0) + 1
    else:
        m.scoreB += 1
        srv = m.current_server_B
        if srv and srv in m.player_points_B:
            m.player_points_B[srv] = m.player_points_B.get(srv, 0) + 1
    ...
    # ── FIX 2: keep events list small to prevent session state bloat
    if len(m.events) > 50:
    ...
    if max(m.scoreA, m.scoreB) >= SET_POINTS and abs(m.scoreA - m.scoreB) >= 2:
        ...
        if m.setsA == 2 or m.setsB == 2:
            ...
            st.session_state.match = m
            save_match_to_file()  # ── FIX 3: persist to disk
            st.rerun()
            return
    ...
    st.session_state.match = m
    save_match_to_file()  # ── FIX 3: persist to disk on every point
    st.rerun()
